...
<title>B2B Cotação Usuários</title>
<meta charset="utf-8">
...
<body>
	...
	<section> <article>
	<form method="post" action="AtualizaUsuario">
		<%
			Usuario usuario = (Usuario) request.getAttribute("usuario");
		%>
		<h1>Usuários</h1>
		<input type=text id=acao name=acao value="editarUsuario" hidden=true>

		<input type=number id=idInput name=idInput hidden=true
			value="<%=usuario.getId()%>" required>
		<p>
			<label> <span>perfil</span><input type=text list=browsers
				id=perfilInput name=perfilInput
				value="<%=usuario.getPerfil() == 1L ? "Colaborador" : "Vendedor"%>">
		...
		<p>
			<label> <span>login</span><input type=text id=loginInput
				name=loginInput value="<%=usuario.getLogin()%>" required></label>
		</p>
		<p>
			<label> <span>senha</span><input type=password id=senhaInput
				name=senhaInput value="<%=usuario.getSenha()%>" required></label>
		</p>

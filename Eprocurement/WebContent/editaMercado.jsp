...
<body>
	...
	<section> <article>
	<form action="AtualizaMercado" method="post">

		<%
			Mercado mercado = (Mercado) request.getAttribute("mercado");
		%>

		<h1>Mercados</h1>
		<input type=text id=acao name=acao value="editarMercado" hidden=true>

		<input type=number id=idInput name=idInput hidden=true
			value="<%=mercado.getId()%>" required>
		<p>
			<label> <span>nome fantasia</span><input type=text
				id=nomeFantasiaInput name=nomeFantasiaInput
				value="<%=mercado.getNomeFantasia()%>" required></label> 
		</p>
		<p>
			<label> <span>razão Social</span><input type=text
				id=razaoSocialInput name=razaoSocialInput
				value="<%=mercado.getRazaoSocial()%>" required></label> 
		</p>
		<p>
			<label> <span>cnpj</span><input type=text id=cnpjInput
				name=cnpjInput value="<%=mercado.getCnpj()%>" required></label> 
		</p>
		<p>
			<label> <span>endereço</span><input type=text
				id=enderecoInput name=enderecoInput
				value="<%=mercado.getEndereco()%>" required></label> 
		</p>
		<p>
			<label> <span>telefone</span><input type=tel name=telInput
				value="<%=mercado.getTelefone()%>" required></label> 
		</p>

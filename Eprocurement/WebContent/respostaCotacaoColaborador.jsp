...
<body>
<jsp:useBean id="listaItensCotacao" class="java.util.ArrayList"
		scope="request"></jsp:useBean>
<jsp:useBean id="listaPrecosCotacao" class="java.util.ArrayList"
		scope="request"></jsp:useBean>
...
<jsp:useBean id="dataTermino" class="java.lang.String"
	...
	<section> <article>
		...
		<h2>
			<label><%out.println(request.getAttribute("nomeFantasia"));%></label>
		</h2>

		<h3>Itens da cotação</h3>
			<%
				for (int i = 0; i < listaItensCotacao.size(); i++) {
			%>
	<form method="post" action="RespostaControlador?acao=responderCotacaoColaborador&idCotacao=<%out.println(request.getAttribute("idCotacao")); %>&i=<%out.println(i);%>&id=<%out.println(((List<Object>) listaItensCotacao.get(i)).get(6));%> ">
		<table>
			...
			<tr "left" bgcolor="<%=i % 2 == 0 ? "white" : "#7fe275"%>">
				<td><% out.println(((List<Object>) listaItensCotacao.get(i)).get(5)); %> </td>
				<td><% out.println(((List<Object>) listaItensCotacao.get(i)).get(2)); %></td>
				<td><% out.println(((List<Object>) listaItensCotacao.get(i)).get(4)); %></td>
				
				<td><label> <input type=number id=precoInput<%out.println(i);%>
						name=precoInput<%out.println(i);%> value=<%
								for (int j = 0; j < listaPrecosCotacao.size(); j++) {
									
									if(((List<Object>) listaItensCotacao.get(i)).get(6) == ((List<Object>) listaPrecosCotacao.get(j)).get(1)){
										out.println(((List<Object>) listaPrecosCotacao.get(j)).get(0));
									} else{
										out.println("");
									}
								}

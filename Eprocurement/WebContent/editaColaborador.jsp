...
<title>B2B Cotação Colaboradores</title>
<meta charset="utf-8">
...
<body>
	...
	<section> <article>
	<form method="post" action="AtualizaColaborador">

		<%
			Colaborador colaborador = (Colaborador) request
					.getAttribute("colaborador");
		%>

		<h1>Colaboradores</h1>

		<input type=text id=acao name=acao value="editarColaborador"
			hidden=true> <input type=number id=idInput name=idInput
			hidden=true value="<%=colaborador.getId()%>" required>

		<p>
			<label> <span>nome</span><input type=text id=nomeInput
				name=nomeInput value="<%=colaborador.getNome()%>" required></label>
		</p>
		<p>
			<label> <span>e-mail </span><input type=email name=emailInput
				value="<%=colaborador.getEmail()%>" required>
			</label>
		</p>

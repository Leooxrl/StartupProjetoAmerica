...
<body>
	...
	<jsp:useBean id="listaProduto" class="java.util.ArrayList"
		scope="request"></jsp:useBean>
	<header> <hgroup>

	<h2>B2B NEGOCIAÇÃO DE PREÇOS ENTRE SUPERMERCADISTAS E FORNECEDORES</h2>
	<ul>
		<li><a class="nav" href="produtos.jsp">Novo Produto</a></li>
	</ul>
	</hgroup> </header>
	...
	<h1>Produtos</h1>
	<input type=text id=acao name=acao value="listarProduto" hidden=true>
	<p>
	<table>
		...
		<tr "left" bgcolor="<%=i % 2 == 0 ? "white" : "#7fe275"%>">
			<td name="idProduto"><%=((Produto) listaProduto.get(i)).getId()%></td>
			<td name="descricaoProduto"><%=((Produto) listaProduto.get(i)).getDescricao()%></td>
			<td><a
				href="AtualizaProduto?acao=editarProduto&id=<%out.println(((Produto) listaProduto.get(i)).getId());%>">editar</a></td>
			<td><a
				href="ExclusaoControlador?acao=excluirProduto&id=<%out.println(((Produto) listaProduto.get(i)).getId());%>">excluir</a></td>
		</tr>

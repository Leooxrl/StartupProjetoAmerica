...
<title>B2B Cotação Vendedores</title>
<meta charset="utf-8">
...
<body>
	...
	<section> <article>
	<form method="post" action="AtualizaVendedor">
		...
		<input type=text id=acao name=acao value="editarVendedor" hidden=true>
		<input type=number id=idInput name=idInput hidden=true
			value="<%=vendedor.getId()%>" required>

		<p>
			<label> <span>nome</span><input type=text id=nomeInput
				name=nomeInput value="<%=vendedor.getNome()%>" required></label>
		</p>
		<p>
			<label> <span>e-mail </span><input type=email name=emailInput
				value="<%=vendedor.getEmail()%>" required>
			</label>
		</p>
		<p>
			<label> <span>telefone</span><input type=tel name=telInput
				value="<%=vendedor.getTelefone()%>" required></label> 
		</p>
